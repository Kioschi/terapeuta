...
import generateSummary from "../../../scripts/users/generateSummary";
...
const Index = ({tableData}) => {
    ...
    const columns = [
        ...
        { field: 'login', headerName: 'Login' ,flex: 1, minWidth: 100},
        ...
        { field: 'lastName', headerName: 'Nazwisko', flex: 1, minWidth: 100},
        {
            ...
            headerName: '',
            renderCell: (params) => (
                <Link href={{ pathname: '/dashboard/users/details', query: { id: params.id } }}>
                    <a>
                        <Button
                            variant="outlined"
                            size="small"
        ...
        },
        {
            ...
            headerName: '',
            renderCell: (params) => (
                <Link href={{ pathname: '/dashboard/users/edit', query: { id: params.id } }}>
                    <a>
                        <Button
                            variant="outlined"
                            size="small"
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Button
                    ...
                    style={{marginLeft: 'auto', marginRight:'auto'}}
                    tabIndex={params.hasFocus ? 0 : -1}
                    onClick={()=> setOpen(prevState => !prevState) & setId(params.id)}
                >
                    Usuń
                </Button>
            ),
            ...
            <Grid container justifyContent={'center'}>
                <Grid item container xs={12} md={8} sx={{m:1}}>
                    ...
                    <Grid item xs={12} sx={{mt: 5}} container justifyContent={'flex-end'}>
                        <Link href={'/dashboard/users/add'}><a><Button variant={'outlined'} size={'small'}>Dodaj użytkownika</Button></a></Link>
                    </Grid>
                    <DataGrid
                        sx={{mt: 5}}
                        autoHeight
...
export async function getServerSideProps(context){
    ...
    const [rows] = await connection.execute('SELECT `id`, `login`, `name`, `lastName` FROM `users` WHERE `deleted` = 0', [])
