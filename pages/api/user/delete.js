    ...
    try{
        ...
        try{
            ...
            if(rows.length>0){
                ...
                await connection.execute('UPDATE `users` SET `deleted` = 1 WHERE `id` = ?', [id])
                await connection.execute('UPDATE `services` SET `deleted` = 1 WHERE `userId` = ?', [id])

                res.status(201).json({message: "Użytkownik usunięty"})
            }
            else
